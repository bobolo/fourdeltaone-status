...
namespace fdocheck.Checks
{
    public class Iw5mCheck : BasicCheck
    {
        ...
        {
            if (!this.NeedToUpdate("serverlist", 30))
        ...
        {
            if (!NeedToUpdate("master", 30)) return;
        ...
        public void CheckNP()
        {
            ...
            try
            {
                ...
                // Send SID check
                ns.Write(preContent, 0, preContent.Length);
                ns.Write(sidContent, 0, sidContent.Length);
                ns.Flush();
                ns.Read(buffer, 0, 64); // first response contains session id again
                ns.Read(buffer, 0, 18); // 18 bytes
                ns.Read(buffer, 0, 18); // 18 bytes
